...
select
  ...
  to_regprocedure('public.create_song_request(uuid,text,text[],text[],text,text,jsonb)') is not null as has_create_song_request_extra_memo,
  exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'song_requests' and column_name = 'memo'
  ) as song_requests_has_memo,
  exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'song_requests' and column_name = 'extra_slots'
  ) as song_requests_has_extra_slots,
  exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'songs' and column_name = 'memo'
  ) as songs_has_memo,
  exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'songs' and column_name = 'extra_slots'
  ) as songs_has_extra_slots,
  ...
  ) as performances_has_start_time,
