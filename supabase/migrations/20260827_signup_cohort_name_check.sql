...
-- (member_cascade_delete.sql 적용 여부와 무관하게 동작)

alter table sessions add column if not exists member_id uuid;
alter table devices add column if not exists member_id uuid;
alter table band_roster add column if not exists member_id uuid;
